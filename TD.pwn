...
// ====================(( #Ayarlar & Tanýtýmlar ))=========================== //
#define MAX_TEXTDRAWS       90			// - Max. textdraw sayýsý. (90 uygundur, ellemeyin.)
#define MSG_COLOR           0xFAF0CEFF	// - Mesaj renkleri.
#define PREVIEW_CHARS       35			// - Textdrawda gösterilecek harf sayýsý.

// - P_Aux ayarlarý
#define DELETING 0
#define LOADING 1

// - P_KeyEdition ayarlarý
#define EDIT_NONE       0
#define EDIT_POSITION   1
#define EDIT_SIZE       2
#define EDIT_BOX        3

// - P_ColorEdition ayarlarý
#define COLOR_TEXT      0
...
{
	bool:T_Created,			// - Textdrawýn durumu.
...
{
	bool:P_Editing,         // - Düzenleme durumu. ( /text açýksa veya kapalýysa.)
	P_DialogPage,           // - Textdraw seçme dialogundaysa.
	P_CurrentTextdraw,      // - Düzenlenen textdraw.
	P_CurrentMenu,          // - Açýk menü.
	P_KeyEdition,           // - Klavye ile düzenleme.
	P_Aux,      		    // - Auxiliar deðiþkeni.
	P_ColorEdition,         // - Renk editörü.
	P_Color[4],             // - RGBA renk yaratýcýsý.
	P_ExpCommand[128],      // - Komut tutar. (Tam çeviremedim. :( )
	P_Aux2                  // - Sadece özel çýkartmalarda kullanýlacak. (Dosya çýkartmalarý.)
};

new tData[MAX_TEXTDRAWS][enum_tData],
	pData[MAX_PLAYERS][enum_pData];
	
new CurrentProject[128];  // - Açýk proje tanýtýmý
...
public OnFilterScriptInit()
{
	printf("\n");
	printf("==========================================");
	printf(" Textdraw Editor baþarýyla açýldý. (0.3x) ");
	printf("                                          ");
	printf(" Developed by:                            ");
...
{
	SendClientMessage(playerid, MSG_COLOR, "Textdraw editörü aktif!");
	SendClientMessage(playerid, MSG_COLOR, "Tasarým yapmak için \"/text\" yazýnýz.");
	return 1;
}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	if(!strcmp("/text", cmdtext))
	{
		...
		if(pData[playerid][P_Editing]) return SendClientMessage(playerid, MSG_COLOR, "[HATA]: Þuan /text komutunu kullanamazsýnýz!");
		else if(!strlen(CurrentProject) || !strcmp(CurrentProject, " "))
		{
		    if(IsPlayerMinID(playerid))
		    {
		        ...
		        SendClientMessage(playerid, MSG_COLOR, "Textdraw tasarlamak için sunucudaki en küçük id'e sahip olmalýsýnýz!");
		    return 1;
		}
		else
		{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else 				PlayerPlaySound(playerid, 1084, 0.0, 0.0, 0.0); // - Vazgeçme sesi.
    
    switch(dialogid)
    {
        case 1574: // - Ýlk dialog.
        {
            if(response) // - Eðer oyuncu kabul ederse.
            {
                strmid(CurrentProject, "", 0, 1, 128);
                
                if(listitem == 0) // - Yeni proje yaratma.
                    ShowTextDrawDialog(playerid, 1);
                else if(listitem == 1) // - Proje açma.
        ...
        case 1575: // - Yeni proje.
        {
            if(response)
            {
                if(strlen(inputtext) > 120) ShowTextDrawDialog(playerid, 1, 1); // - Çok uzun.
					...
					inputtext[0] == ' ' )
						ShowTextDrawDialog(playerid, 1, 3); // - Ýllegal karakterler.
						
                else // - Tamamdýr, yeni dosya yaratýldý.
                {
                    new filename[128];
                    format(filename, sizeof(filename), "%s.tde", inputtext);
                    if(fexist(filename)) ShowTextDrawDialog(playerid, 1, 2); // - Dosya önceden yaratýlmýþ!
                    else
                    {
	                    ...
	                    new tmpstr[128];
	                    format(tmpstr, sizeof(tmpstr), "Þuan '%s' projesi üzerinde çalýþýyorsunuz.", filename);
	                    SendClientMessage(playerid, MSG_COLOR, tmpstr);
	                    
	                    ShowTextDrawDialog(playerid, 4); // - Ana menüyü gösterme.
			 		}
                }
            }
            else
        ...
        case 1576: // - Proje yükleme/silme.
        {
            if(response)
            {
                if(listitem == 0) // - Özel dosya ismi.
                {
        ...
        case 1578: // - Textdraw seçme.
        {
            if(response)
            {
                if(listitem == 0) // - Yeni textdraw
                {
					...
					{
					    SendClientMessage(playerid, MSG_COLOR, "Daha fazla textdraw yaratamazsýnýz!");
					    ShowTextDrawDialog(playerid, 4, pData[playerid][P_DialogPage]);
					}
					else
					{
						new string[128];
	                    format(string, sizeof(string), "Textdraw #%d baþarýyla yaratýldý!", pData[playerid][P_CurrentTextdraw]);
	                    SendClientMessage(playerid, MSG_COLOR, string);
					}
                }
                else if(listitem == 1) // - Proje çýkartma.
                {
                ...
                else if(listitem == 2) // - Proje kapatma.
                {
                    if(IsPlayerMinID(playerid))
                    {
	                    ...
	                    new string[128];
	                    format(string, sizeof(string), "'%s' projesi kapatýldý.", CurrentProject);
	                    SendClientMessage(playerid, MSG_COLOR, string);
					...
					{
					    SendClientMessage(playerid, MSG_COLOR, "Sadece sunucudaki en küçük id'e sahip kiþi proje yönetebilir.");
					    ShowTextDrawDialog(playerid, 4);
					}
                }
                else if(listitem <= 10) // - Textdraw seçme.
                {
                    new id = 3;
                    ...
                    new string[128];
                    format(string, sizeof(string), "Textdraw #%d düzenleniyor.", pData[playerid][P_CurrentTextdraw]);
                    SendClientMessage(playerid, MSG_COLOR, string);
                }
                else
                {
        ...
        case 1579: // - Ana düzenleme menüsü.
        {
            if(response)
            {
                switch(listitem)
                {
	                ...
	                case 7: // - Dýþ çizgi düzenleme.
	                {
	                    ShowTextDrawDialog(playerid, 20);
	                }
	                case 8: // - Box düzenleme.
	                {
	                ...
	                case 9: // - TextDrawSetSelectable ayarý.
	                {
	                ...
	                case 10: // - Textdraw kopyalama.
	                {
						...
						if(pData[playerid][P_CurrentTextdraw] != -1)
						{
						    SendClientMessage(playerid, MSG_COLOR, "Daha fazla textdraw yaratamazsýn!");
						    ShowTextDrawDialog(playerid, 5);
						}
						else
						{
							new string[128];
		                    format(string, sizeof(string), "Textdraw #%d baþarýlý bir þekilde Textdraw #%d olarak kopyalandý.", from, to);
		                    SendClientMessage(playerid, MSG_COLOR, string);
						}
	                }
        ...
        case 1580: // - Proje silme iþlemi onay dialogu.
        {
            if(response)
            {
                new filename[128];
				...
				format(filename, sizeof(filename), "'%s' isimli proje baþarýyla silindi.", filename);
        ...
        case 1581: //- Proje silme iþlemi onayý.
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d baþarýyla silindi.", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
        ...
        case 1582: // - Textdraw metni deðiþtirme.
        {
            if(response)
            {
        ...
        case 1583: // - Textdraw pozisyonu deðiþtirme.
        {
            if(response)
            {
                switch(listitem)
                {
                    case 0: // - Koordinat girerek taþýma.
                    {
                        pData[playerid][P_Aux] = 0;
                        ShowTextDrawDialog(playerid, 10, 0, 0);
                    }
                    case 1: // Hareket ettirerek taþýma.
                    {
                        new string[512];
						...
						SendClientMessage(playerid, MSG_COLOR, "Textdrawý hareket ettirmek için ok tuþlarýný kullanýnýz. Shift tuþu daha hýzlý hareket ettirir ve Enter tuþu bitirir.");
        ...
        case 1584: // - Pozisyonu manuel ayarlama.
        {
            if(response)
            {
                ...
                {
                    if(pData[playerid][P_Aux] == 0) // - Eðer X düzenlenirse;
                    {
                    ...
                    else if(pData[playerid][P_Aux] == 1) // - Eðer Y düzenlenirse;
                    {
						...
						SendClientMessage(playerid, MSG_COLOR, "Textdraw baþarýyla hareket ettirildi.");
                    }
                }
            }
            else
            {
                if(pData[playerid][P_Aux] == 1) // - X düzenlendikten sonra Y düzenleme.
                {
                    pData[playerid][P_Aux] = 0;
                    ShowTextDrawDialog(playerid, 10, 0, 0);
                }
                else // - X editlendikten sonra menüye dönme.
                {
                    ShowTextDrawDialog(playerid, 9);
                }
            }
        ...
        case 1585: // -
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn duruþu %d olarak deðiþtirildi.", pData[playerid][P_CurrentTextdraw], listitem+1);
                SendClientMessage(playerid, MSG_COLOR, string);
        ...
        case 1586: // - Harf aralýðý.
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn harf aralýðý %d olarak deðiþtirildi.", pData[playerid][P_CurrentTextdraw], listitem);
                SendClientMessage(playerid, MSG_COLOR, string);
        ...
        case 1588: // - Textdraw rengi: Hex rengi.
        {
        	if(response)
            {
                ...
                if(inputtext[0] == '0' && inputtext[1] == 'x') // - 0xFFFFFF formatý kullanýlýyor.
                {
                ...
                else if(inputtext[0] == '#') // - #FFFFFF formatý kullanýlýyor.
                {
                ...
                else // - FFFFFF formatý kullanýlýyor.
                {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn rengi deðiþtirildi.", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
		...
		case 1589: // - Textdraw rengi: RGBA editörü.
        {
            if(response)
            {
                ...
                else
                {
                    ...
                    if(pData[playerid][P_Aux] == 3) // - Renk giriþi bitirildi ise;
                    {
                        // - RGBA kayýtlarý.
                        if(pData[playerid][P_ColorEdition] == COLOR_TEXT)
            ...
            {
                if(pData[playerid][P_Aux] >= 1) // - Eðer renk düzenledi ise menüye dönme;
                {
                    pData[playerid][P_Aux] -= 1;
                    ShowTextDrawDialog(playerid, 15, pData[playerid][P_Aux], 0);
                }
                else // - Menüye geri dönme. (Hiç düzenlemeden.)
                {
                    ShowTextDrawDialog(playerid, 13);
                }
            }
        ...
        case 1590: // - Textdraw rengi: Renk seçme.
        {
            if(response)
            {
                new col;
        ...
        case 1591: // - Textdraw fontu deðiþtirme.
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn fontu %d olarak deðiþtirildi.", pData[playerid][P_CurrentTextdraw], listitem);
                SendClientMessage(playerid, MSG_COLOR, string);
                if(listitem == 4)
                {
                    SendClientMessage(playerid,MSG_COLOR, "Þuan font4'ü kullanýyorsunuz.");
                    SendClientMessage(playerid,MSG_COLOR, "Bu özelliði kullanýrken TextDrawBox açýk olmalýdýr.");
                    SendClientMessage(playerid,MSG_COLOR, "TextDrawBox boyutu texture'nin boyutunu ayarlar.");
                    SendClientMessage(playerid,MSG_COLOR, "Bu fonksiyon {FFFFFF}irinel1996 {FAF0CE}tarafýndan eklenmiþtir.");
				}
                ShowTextDrawDialog(playerid, 5);
            }
            else
        ...
        case 1592: // - Textdraw yazý büyüklüðü: Manuel ve hareket ettirerek.
        {
            if(response)
            {
                switch(listitem)
                {
                    case 0: // - Manuel kullaným.
                    {
                        pData[playerid][P_Aux] = 0;
                    ...
                    case 1: // - Hareket ettirme.
                    {
						...
						GameTextForPlayer(playerid, string, 9999999, 3);
						SendClientMessage(playerid, MSG_COLOR, "Textdrawý yeniden boyutlandýrmak için ok tuþlarýný kullanýnýz. Shift tuþu daha hýzlý hareket ettirir ve Enter tuþu bitirir.");
        ...
        case 1593: // - Manuel olarak textdraw yazý büyüklüðü ayarlama.
        {
            if(response)
            {
                ...
                {
                    if(pData[playerid][P_Aux] == 0) // - X düzenleme;
                    {
                    ...
                    else if(pData[playerid][P_Aux] == 1) // - Y düzenleme;
                    {
						...
						SendClientMessage(playerid, MSG_COLOR, "Textdraw baþarýyla yeniden boyutlandýrýldý.");
                    }
                }
            }
            else
            {
                if(pData[playerid][P_Aux] == 1) // - Y düzenlenince X'e gönderme.
                {
                    pData[playerid][P_Aux] = 0;
                    ShowTextDrawDialog(playerid, 19, 0, 0);
                }
                else // - X bitince menüyü kapatma.
                {
                    ShowTextDrawDialog(playerid, 18);
                }
            }
        ...
        case 1594: // - Ana dýþçizgi menüsü:
        {
            if(response)
            {
				switch(listitem)
				{
				    case 0: // - Dýþçizgi açma.
				    {
				        ...
				        SendClientMessage(playerid, MSG_COLOR, "Textdraw dýþçizgisi açýldý.");
				    }
					case 1: // - Gölge deðiþtirme.
					{
                        ShowTextDrawDialog(playerid, 21);
					}
					case 2: // - Renk deðiþtirme.
					{
					...
					case 3: // - Bitirme.
	                {
	                    SendClientMessage(playerid, MSG_COLOR, "Dýþçizgi düzenlemesi bitirildi.");
	                    ShowTextDrawDialog(playerid, 5);
	                }
				}
            }
        ...
        case 1595: // - Dýþçizgi gölgesi.
        {
            if(response)
            {
                if(listitem == 6) // - Gölge seçme.
                {
                ...
                else
                {
					...
					new string[128];
	                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn dýþçizgi gölgesi %d olarak deðiþtirildi.", pData[playerid][P_CurrentTextdraw], listitem);
	                SendClientMessage(playerid, MSG_COLOR, string);
                }
            }
            else
        ...
        case 1596: // - Dýþçizgi gölgesi deðiþtirme.
        {
            if(response)
            {
                ...
                else
                {
					...
					new string[128];
	                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn dýþçizgi gölgesi %d olarak deðiþtirildi.", pData[playerid][P_CurrentTextdraw], strval(inputtext));
	                SendClientMessage(playerid, MSG_COLOR, string);
                }
            }
            else
        ...
        case 1597: // - TextDrawBox açma/kapatma.
        {
            if(response)
            {
                switch(listitem)
                {
                    case 0: // - Box açma.
                    {
						...
						SendClientMessage(playerid, MSG_COLOR, "TextDrawBox aktifleþtirildi, düzenleme açýk!");

						ShowTextDrawDialog(playerid, 24);
                    }
                    case 1: // - Box pasifleþtirme.
                    {
						tData[pData[playerid][P_CurrentTextdraw]][T_UseBox] = 0;
						UpdateTextdraw(pData[playerid][P_CurrentTextdraw]);
						SaveTDData(pData[playerid][P_CurrentTextdraw], "T_UseBox");
						
						SendClientMessage(playerid, MSG_COLOR, "TextDrawBox pasifleþtirildi!");
            ...
            if(response)
            {
                switch(listitem)
                {
                    ...
                    case 1: // - Box boyutu.
                    {
						...
						GameTextForPlayer(playerid, string, 9999999, 3);
						SendClientMessage(playerid, MSG_COLOR, "TextdrawBox boyutunu ayarlamak için ok tuþlarýný kullanýnýz. Shift tuþu daha hýzlý hareket ettirir ve Enter tuþu bitirir.");
        ...
        case 1606: // - TextDrawSelectable özelliði.
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn seçilebilirliði açýldý.", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
            else
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d isimli textdrawýn seçilebilirliði kapatýldý.", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
        }
        case 1599: // - Çýkartma menüsü
        {
            if(response)
            {
                switch(listitem)
                {
                    ...
                    case 2: // PlayerTextDraw modu. [ADRI1 tarafýndan eklendi.]
                    {
...
{
	/* RGB renk dönüþtürücü.
	@red: Kýrmýzý  deðeri.
	@green: Yeþil deðeri.
	@blue: Mavi deðeri.
	@alpha: Saydamlýk.
	
	Return: Dönüþtürülmüþ renk sonucu çýkacaktýr.
	*/
...
/** Yapýmcý: DRACOBLUE
 *  Ýþlevi: Hex renklerini normal renge çevirir.
 *  @param string
 */
...
{
	/*Oyuncunun en düþük id'e sahip olup olmadýðýný kontrol eder.
	
      - playerid: Kontrol için.
      - Returns: true == En düþük ide sahip. / false == En düþük ide sahip deðil.
	*/
